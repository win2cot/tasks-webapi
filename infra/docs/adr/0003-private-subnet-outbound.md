...
- **欠点**:
  ...
  - 構成が複雑化する
- **リスク・未知数**:
  - prod の定常トラフィック量が読めるまで損益分岐が確定しない
...
Private Subnet の default route(`0.0.0.0/0`)を NAT Gateway に向け、S3 向けトラフィックは Gateway Endpoint で NAT を迂回させる。Interface Endpoint(選択肢 d)は **prod 構築時(Post-Sprint-0)に定常トラフィック量 × セキュリティ要件で再評価**する。NAT instance(選択肢 e)は EC2 不使用原則により不採用とする。
...
捨てた利点として、選択肢 (b) の最高セキュリティと安い per-GB は魅力だが、dev では固定費の高さ・外部不達・Phase 2 の手戻りがそれを上回る。prod で再評価する。
...
- single-AZ NAT のため、当該 AZ 障害時に Private Subnet の egress が断たれる(dev は非 HA 許容、prod で multi-AZ を再評価)
...
- AWS API 呼び出しがインターネット往復になる(セキュリティ・レイテンシとも dev は許容、prod で Interface Endpoint を再評価)
