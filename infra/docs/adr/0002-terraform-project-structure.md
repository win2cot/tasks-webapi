...
- S0Infra-2(#239): state backend 構築 / S0Infra-9(#246): outbound 経路 ADR-0003
...
- `infrastructure-plan.md` v5 §3.3 のディレクトリ図と概ね整合(stg / prd を「雛形のみ Post-Sprint-0」とする点も一致)。ただし §3.3 の module 一覧(10 個)は `security_group` を独立列挙していないため、本 ADR で追加した `security_group` を §3.3 にも追記するのが望ましい(軽微な doc 同期、別途実施可)
...
- `infra/docs/adr/0001-private-dns-for-rds.md`(隣接 ADR、env 毎独立 PHZ)
- `infra/docs/adr/0003-...`(予定): Private Subnet outbound 経路選定(S0Infra-9 / #246)
