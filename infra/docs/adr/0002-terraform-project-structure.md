...
env 毎に独立した state を持つ(layer 別分割はしない)。backend は S3 + `use_lockfile`(S3 ネイティブロック、DynamoDB 不採用。実装は S0Infra-2)、key 規約は `tasks/<env>/terraform.tfstate`。layer 分割は prd 構築時(Post-Sprint-0)に再評価する。
...
各 env root に `versions.tf` を置く。`required_version`(Terraform 本体)は `use_lockfile` 要件のため `>= 1.11` を下限固定、`required_providers`(`hashicorp/aws ~> 5.x`)は悲観的制約 `~>` で固定し、`.terraform.lock.hcl` を env ごとにコミットする。module 側は `required_providers` で source 宣言のみ行い、version 制約は root に委譲する。具体バージョンは Sprint 0 着手時点の最新 stable で確定する。
...
```hcl
terraform {
  required_version = ">= 1.11"   # use_lockfile(S3 ネイティブロック)要件
...
terraform {
  backend "s3" {
    bucket       = "dgz48-tfstate"   # 複数 project 共用の中立名(S0Infra-2)
    key          = "tasks/dev/terraform.tfstate"
    region       = "ap-northeast-1"
    encrypt      = true
    use_lockfile = true              # S3 ネイティブロック(DynamoDB 不採用)
  }
}
```

S3 bucket は `dgz48-tfstate`(複数 project の state を集約する中立名、グローバル一意)を bootstrap で 1 個だけ作成する。ロックは `use_lockfile = true`(Terraform 1.11 GA の S3 ネイティブロック)を採用し、DynamoDB table は作らない(deprecated のため不採用)。platform / tasks の 2 state は同一 bucket を key prefix(`platform/dev/...` / `tasks/dev/...`)で分離する(ADR-0004)。bootstrap 手順は `infra/docs/bootstrap.md` 参照。
