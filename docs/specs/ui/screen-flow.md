...
- **対応 API**(#355 / FM-G4 で確定、2026-06-02): 専用エンドポイント **`GET /api/dashboard/tasks`**(OpenAPI v1.6.0、`operationId: getDashboardTasks`)で 4 セクション分を 1 回で取得する。レスポンスは `DashboardTaskSections`(`overdue` / `today` / `upcoming` / `completedToday` の 4 配列)で、**サーバがセクション別ソート済**で返すためクライアントは描画のみ(再振り分け・再ソート不要)。対象は「ログイン中ユーザーが所有者または担当者」かつ §6.2.1 認可フィルタ通過後のタスク(`ownerOrAssignee=me` 相当はパスの意味論に内包、クエリでは渡さない)。「今日」はサーバ側システム日付(JST)で確定。(3) これから の先読み日数は `dueWithinDays`(integer、既定 3、Phase 2 で個人通知設定と統合し可変化余地)。
...
- **編集可能項目**(セルクリック → その場で編集):
  ...
  - **期限**(date picker)→ A-29 `PATCH /api/tasks/{id}`(ADR-0014/ADR-0012)。**所有者のみ**。S-03 では期限変更で所属セクションが切り替わる
  - **担当者**(プルダウン)→ A-29 `PATCH /api/tasks/{id}`。**所有者のみ**
  - **優先度**(プルダウン)→ A-29 `PATCH /api/tasks/{id}`。**所有者のみ**
  - **タイトル**(セル内テキスト入力)→ A-29 `PATCH /api/tasks/{id}`。**所有者のみ**
  - **説明**(ポップオーバー型 textarea。行高を超えるためインラインではなくセルクリックで小さなオーバーレイ展開)→ A-29 `PATCH /api/tasks/{id}`。**所有者のみ**
- **行内編集の対象外項目**(ドロワー編集のみ):
...
4. **OpenAPI A-14 PUT の部分更新化の議論**: 行内編集多用時は `PUT /api/tasks/{id}`(全項目)よりも `PATCH /api/tasks/{id}`(部分更新)の方が UX・帯域・コンフリクト面で有利。A-16 / A-17 で既に個別 PATCH があるが、期限・担当者・優先度・タイトル・説明の 5 項目を一括カバーする PATCH が必要かを **Issue #329 で議論 → 案 (1) 採用で確定(2026-06-01)= `PATCH /api/tasks/{id}` 新設 + A-14 PUT 廃止**。後続として (i) `JsonNullable<T>` を採用する DTO 規約 ADR → ADR-0014 Accepted(Issue #331 クローズ)、(ii) ETag / If-Match 楽観ロック ADR(`tasks.version` カラム新設)→ ADR-0012 Accepted(Issue #332 クローズ)、(iii) 監査ログ field-by-field 差分の方式選定 ADR → ADR-0013(Issue #333 クローズ)、(iv) OpenAPI v1.5.x 反映(PUT 廃止 + PATCH 新設)→ **Issue #334 クローズ(本 PR)。OpenAPI v1.5.0 に更新、基本設計書 §5.1 API 一覧に A-29 `PATCH /api/tasks/{id}` 新設 + A-14 PUT 廃止行削除で改訂完結**
