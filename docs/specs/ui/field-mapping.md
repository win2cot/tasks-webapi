...
Version 1.3 / 2026-06-03 / Issue #153 / 親 Issue #149(Sprint 0 画面設計)
...
| 1.1 | 2026-06-02 | FM-G4(#355)の方針を **案 B(専用 `GET /api/dashboard/tasks`)** で確定。§1.2 の 4 セクション一括取得クエリ行(✗)を解消、§5 / §8 の FM-G4 行を「確定」に更新。OpenAPI は v1.6.0(`getDashboardTasks` + `DashboardTaskSections` 追加) | 開発チーム |
| 1.2 | 2026-06-02 | FM-G5(#356)の方針を **案 B(MVP は client 派生集計)** で確定。§2.3 のバッジ行に方針注記を追加、§5 / §8 の FM-G5 行を「確定(案 B)」に更新、§5.1 に観察事項 O3(再評価トリガ)を追記。OpenAPI 変更なし(本 Issue は観察事項として close) | 開発チーム |
| 1.3 | 2026-06-03 | FM-G3(#358)を解消。`Tenant` に `userCount` / `taskCount` を追加(OpenAPI v1.7.0)。§4.2 / §4.3 の ✗ 行を ✓ に更新、§5 / §8 の FM-G3 行をストライクスルー、§7 完了条件を全件解消・確定済みに更新 | 開発チーム |
...
| 4 セクション一括取得クエリ(`GET /api/dashboard/tasks?dueWithinDays=3`) | `DashboardTaskSections`(overdue / today / upcoming / completedToday の 4 配列) | ✓(FM-G4 / #355 案 B で解消、OpenAPI v1.6.0) |
...
| 画面の表示項目 | OpenAPI | 充足 |
|---|---|---|
| 月単位の日別タスク件数(期限切れ / 当日内訳) | 月別集計 endpoint 無し → **案 B: client 派生集計で MVP 確定**(#356) | △(MVP は client 集計で充足、§5.1 O3) |

> **FM-G5 方針(#356、2026-06-02 確定 / 案 B)**: カレンダー日別件数バッジは MVP では専用集計 endpoint を設けず、当月分の `listTasks`(認可通過後のタスクのみ返る)を client で `dueDate` 別にバケットして件数を算出する。`listTasks` が認可済タスクのみ返すためバッジ件数は自動的に認可済件数になる(decision.md §4 の API 確定待ち論点を解消)。当月レンジは `targetDate`(単日)では取れないため、当月分を取得して client 側で月内に絞り込む。MVP 件数規模(テナント 1 日数十件以下、月数百件程度)では妥当。再評価トリガは §5.1 O3 参照。
>
...
| 所属ユーザー数 | `Tenant.userCount` | ✓ |
| タスク数 | `Tenant.taskCount` | ✓ |
| 状態フィルタ(ACTIVE / SUSPENDED / すべて) | `listTenants` `status` query parameter | ✓ |
| 名前検索(部分一致) | `listTenants` `keyword` query parameter | ✓ |
| ページング(50 件 / ページ) | `listTenants` → `TenantPage`(`page` / `size` parameter、`content` + `totalElements` + `totalPages` + `number` + `size`) | ✓ |
...
| ~~**FM-G2**~~ | ~~必須~~ | ~~S-13~~ | ~~`listTenants`(A-04)に **状態フィルタ / 名前検索 / ページング** の parameter が無く、レスポンスも `Tenant` 配列直返しで Page 構造が無い~~ | `status` / `keyword` / `page` / `size` parameter を追加し、レスポンスを `TenantPage`(`content` + `totalElements` + `totalPages` + `number` + `size`)に変更 (#357) | **解消済み** |
| ~~**FM-G3**~~ | ~~必須~~ | ~~S-13 / S-14~~ | ~~`Tenant` スキーマに **`userCount` / `taskCount`** が無く、一覧の所属ユーザー数 / タスク数列・詳細メトリクスが取得不可~~ | `Tenant` に `userCount` / `taskCount`(read-only)を追加 (#358) | **解消済み** |
| **FM-G4** | 確定(案 B) | S-03 | ダッシュボード 4 セクションを 1 回で取るための **`ownerOrAssignee=me`(owner OR assignee の論理和)/ これから(+N 日範囲)/ 本日完了(`completedAt` = 当日)** が `listTasks` に無い(現状は `ownerId` と `assigneeId` が AND、`targetDate` は単日、完了は `status=DONE` 全件) | **案 B 採用(#355、2026-06-02 確定)**: 専用 `GET /api/dashboard/tasks` を新設、4 セクション(`overdue` / `today` / `upcoming` / `completedToday`)を `DashboardTaskSections` で一括返却。`upcoming` の先読み日数は `dueWithinDays`(integer、既定 3)で可変。サーバがセクション別ソート済で返す。OpenAPI v1.6.0 反映済 | (3) これからは複数回呼びで代替可だが、(4) 本日完了は `status=DONE` 全件取得になりコスト過大 → 実質不可 |
| **FM-G5** | 確定(案 B) | S-04 | 案 B 左サイドカレンダーの **月単位日別件数バッジ** を生成する集計 endpoint が無い | **案 B 採用(#356、2026-06-02 確定)**: MVP は専用 endpoint を設けず、当月 `listTasks`(認可済タスクのみ)を client で日別集計。OpenAPI 変更なし。本 Issue は観察事項として close、再評価トリガは §5.1 O3 | 当月分取得 + client 集計で充足(MVP 件数規模では妥当、件数増で再評価) |
...
- **O3(カレンダー件数バッジの集計方式 / FM-G5 #356)**: MVP は案 B(client 派生集計)を採用し、専用集計 endpoint は設けない。再評価トリガは (a) テナント当たり月間タスク量が数百件を大きく超え、当月全件取得 + client 集計の負荷・転送量が問題化したとき、(b) #154 がバッジ内訳(期限切れ / 当日 / 合算)を server 集計前提の仕様で確定したとき。いずれかに該当したら `GET /api/tasks/calendar?month=YYYY-MM` 相当の集計 endpoint(案 A)を再検討する。
...
| 不足項目が 0 件 or OpenAPI 更新の追加 Issue として起票済み | FM-G1(#354)/ FM-G2(#357)/ FM-G3(#358)解消済み、FM-G4(#355、案 B)/ FM-G5(#356、案 B)方針確定済み。すべて Issue 起票済み(§8) |
...
| ~~FM-G2~~ | ~~#357 listTenants フィルタ/検索/ページング + TenantPage~~ | ~~必須~~ → **解消済み** |
| ~~FM-G3~~ | ~~#358 Tenant に userCount/taskCount~~ | ~~必須~~ → **解消済み** |
| FM-G4 | #355 ダッシュボード4セクション取得クエリ → **案 B 確定(`GET /api/dashboard/tasks`、OpenAPI v1.6.0)** | 確定 |
| FM-G5 | #356 カレンダー日別件数バッジ → **案 B 確定(client 派生集計、OpenAPI 変更なし、観察事項として close)** | 確定 |
...
- API 契約: `api/openapi.yaml` v1.7.0
