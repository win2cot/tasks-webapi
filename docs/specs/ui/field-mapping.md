...
Version 1.1 / 2026-06-02 / Issue #153 / 親 Issue #149(Sprint 0 画面設計)
...
| S-05・S-06 タスク詳細・編集 | `getTask` / `patchTask` / `changeStatus` / `changeVisibility` / `listStakeholders` / `addStakeholder` / `removeStakeholder` / `deleteTask` | `TaskDetailPartial` / `TaskPatchRequest` / `Stakeholder` / `UserSummary` |
...
### 3.1 表示項目 → `getTask`(A-12)→ `TaskDetailPartial`

| 画面の表示項目 | OpenAPI フィールド(`TaskDetailPartial`) | 充足 |
|---|---|---|
| タイトル | `title` | ✓ |
| 説明 | `description`(nullable) | ✓ |
| ステータス | `status` | ✓ |
...
| **所有者(名前 + canX 認可判定の基点)** | **無し**(`owner` / `ownerId` フィールドが存在しない) | ✗ → **FM-G1** |
| **担当者(名前)** | `assigneeId`(integer)のみ。`UserSummary` オブジェクト無し | △ → **FM-G1**(名前は `listTenantUsers` 突き合わせで暫定可だが非効率) |
| **編集可否フラグ** `editable` | **無し**(`Task` には在るが `TaskDetailPartial` に無い) | ✗ → **FM-G1** |
| **削除可否フラグ** `deletable` | **無し**(同上) | ✗ → **FM-G1** |
...
|---|---|---|---|---|---|
| **FM-G1** | 必須 | S-05 / S-06 | `getTask`(`TaskDetailPartial`)に **`owner`(UserSummary)/ `assignee`(UserSummary)/ `editable` / `deletable`** が無く、所有者・担当者名の表示と ADR-0005 の 3 役割認可判定(`canFields` 等)ができない。一覧用 `Task` には在るのに詳細用にだけ欠落している(#279 は DB 列 `created_by`/`updated_by` 追加のみで本件未対応) | `getTask` のレスポンスを `Task` 相当に統一する(`owner` / `assignee` を `UserSummary` 化 + `editable` / `deletable` 追加)。あるいは `TaskDetailPartial` を `TaskDetail` に昇格して 4 フィールドを追加 | 不可(`ownerId` 自体が無く client では認可判定不能) |
...
| `TaskDetailPartial.tenantId` | 内部識別、画面未表示 |
...
| 不足項目が 0 件 or OpenAPI 更新の追加 Issue として起票済み | 不足 5 件(FM-G1〜G5)。OpenAPI 更新 Issue を別途起票(§8) |
...
| ID | Issue | 重要度 |
|---|---|---|
| FM-G1 | #354 getTask レスポンス拡張(owner/assignee/editable/deletable) | 必須 |
