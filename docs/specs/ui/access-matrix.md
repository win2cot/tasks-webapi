...
| 9 | **所有者が自タスクを `visibility = PRIVATE` に変更した後、過去に登録した関係者レコード** | **200 OK**(visibility 変更成功。`task_stakeholders` レコードは同処理内で CASCADE 削除) | `STAKEHOLDER_PURGED` | PRIVATE では関係者リストが参照権限に影響しないため、不要なリスト残留を防ぐ(基本設計書 §4.2.5 / 要件定義書 §3.4.4) |
