...
| WAF(stg/prd の ALB) | 時間 + 送信元 IP | requestId の相関なし(infra ADR-0006、REGIONAL Web ACL のログ)。app アクセスログ行(`HTTP_REQUEST`)が同一時間窓・IP の相関先 |

**再評価トリガ**: API を CloudFront 配下に入れる構成変更(edge WAF 等)を行う場合、CloudFront request id の伝播を含めて本節を再設計する。**現時点(infra ADR-0006、#498)では「API は CloudFront 配下に入れない / WAF は stg/prd の ALB に REGIONAL Web ACL」と確定済**のため、本節は不変。CloudFront 配下化の検討トリガは infra ADR-0006 §6 を参照。
