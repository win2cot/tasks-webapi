...
- 採用すると決まった ADR は原則 **不変**。後で覆す場合は新規 ADR(Status: Supersedes ADR-NNNN)を立てる。
  - 例外: 作成直後で他 ADR / Issue / 実装がまだ依存していない、または考慮漏れ・誤りの軽微な瑕疵補完にとどまる場合は、Supersede でなく当該 ADR を直接改訂してよい(改訂日と理由をヘッダに注記)。確立・被参照の ADR は引き続き不変(= Supersede / 追補で対応)。
