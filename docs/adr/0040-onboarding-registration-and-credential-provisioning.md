...
#### 3.5.1 検討した代替: 会員登録とテナント作成の 1 トランザクション結合(却下)

「登録とテナント作成を 1 トランザクションで原子的に」という案も検討したが **却下**した。理由:

- **本当に跨ぐ境界は users↔tenant ではなく project DB↔Keycloak**: `users` 行も `tenant`/`user_tenants` 行も同一 project DB にあり、束ねるのは元々容易。一方 credential 設定は Keycloak Admin API のリモート呼び出しで、DB の `@Transactional` には入れられない(分散トランザクションは持たない)。結合しても **project DB↔Keycloak の 2 系統境界は消えず**、補償は依然必要。結合は「DB 側の書き込みをまとめる」だけで危険な境界を動かせない。
- **失敗状態を悪化させる**: ADR-0006 §3.3 の固定順序「project 先 → Keycloak 後」を前提にすると、結合案では DB(users + tenant + user_tenants)をコミット後に Keycloak credential write が失敗した場合、**フル構築済みのテナントに唯一の管理者がログインできない**(孤立テナント + 認証不能オーナー)という最悪の宙吊りになる。分離案なら登録時 Keycloak 失敗 → 下流は何も作られず、登録を再試行するだけ(最悪でも未所属アカウント、回復可)。
- **未認証パスの二重化**: テナント作成は現状 `@AuthenticationPrincipal(expression = "id")` 由来(ログイン済み前提)。サインアップ時点では JWT が無く、新規ユーザー id を直接渡す別経路が要る。
- **共有プリミティブの分岐**: 登録プリミティブは Flow A(招待=テナント作成なし)と共有(§3.3)。結合すると Flow B だけ分岐が増える。
- **UX は結合せずに回収できる**: 「1 サブミット」の利点は、サインアップ時にテナント名も取得し初回ログイン直後に best-effort で `POST /api/tenants` を自動実行(失敗時は未所属ランディングにフォールバック)すれば、**分離の堅牢性のまま結合相当の UX** を実現できる。
