...
- **Flow B(創業者 / セルフサインアップ)** — **double opt-in**(§3.5 の到達確認を内在化):
...
### 3.5 メール到達確認(email reachability / verification)
...
- password reset / MFA setup は **Keycloak ホスト型のまま**(ADR-0017 §3.2、追加実装なし)。email の初回到達確認は §3.5 でオンボーディングに内在化する(Keycloak `VERIFY_EMAIL` action には依存しない)。
...
- email 到達確認が両フローの構造に内在化(§3.5)。未確認 email でアカウントが作られず、`emailVerified=true` を登録時に確定できる。
