...
> 補足(2026-06-07): イメージ作成手段は [ADR-0018](0018-container-image-build-with-boot-build-image.md) で `bootBuildImage`(Cloud Native Buildpacks)に確定し、Dockerfile は作成しない。本節の「Dockerfile を Native build ベースに変更」は「コンテナイメージを Native build ベースに切り替え」と読み替える。Sprint 1 予定だった nativeBuild CI job は起票漏れにより Sprint 2 で回収する(#490)。
