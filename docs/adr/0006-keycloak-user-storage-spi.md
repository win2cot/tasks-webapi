# ADR-0006: Keycloak User Storage SPI を実装し、users テーブルを profile/directory の SoT として運用する(read-only SPI + email のみ writable、2026-06-04 改訂)

- **Status**: Accepted(**2026-06-04 改訂**: 採用案を D'(フル writable SPI)→「A(read-only)+ email のみ writable 例外」へ narrow、属性所有ルールを明文化、Keycloak runtime を platform 共有 IdP へ。§3 / §3.1 / §3.3 / §5 + `infra/docs/adr/0004` 参照。本文中「writable SPI / フル CRUD」は email-only writable と読み替える)
- **Date**: 2026-05-24
- **Deciders**: 開発チーム
...
**採用**: 選択肢 A をベースに **email 属性のみ writable 例外**を加えたハイブリッド — 本システム `users` が **profile / directory の SoT**、**資格情報・MFA factor は Keycloak**、SPI は profile を **read-only federate + email のみ write 戻し**。

> **2026-06-04 改訂**: 当初は選択肢 D'(フル writable SPI、Keycloak Console から name / email / locale を編集 + Console 作成)を採用していたが、属性所有を「identity・検証グレード(Keycloak)/ それ以外(project)」で切り分け、**SPI の writable 範囲を email のみに narrow**した(§3.1 / §4)。あわせて Keycloak runtime を **platform 共有 IdP**(multi-realm、per-realm SPI)へ移す(`infra/docs/adr/0004` 改訂)。本案件の `users` に **phone 列が無い**ため SMS-OTP は対象外、MFA は TOTP / WebAuthn / email-OTP で提供する。
...
| `UserRegistrationProvider` | user の作成・削除 | 実装(**admin / 障害時リカバリ経路のみ**。主経路は本システム招待 API。削除は 3.4 の論理削除に変換) |
| `CredentialInputValidator` | credential 検証 | **実装しない**(資格は Keycloak の標準 credential store、3.3 参照) |

write 範囲は **read-only federation + email のみ writable**(2026-06-04 改訂、当初フル CRUD):

| 操作 | SPI 経由 | 本システム API 経由 |
|---|---|---|
| user 作成 | △(admin / 障害時のみ、後述の制約あり) | ✓(**主経路**。招待 API、tenant 割当て等の業務ロジック) |
| user 読み取り(認証時・profile) | ✓(SPI が users を **read-only** federate) | — |
| **email 更新** | ✓(**Keycloak Update-Email の到達検証を通った場合のみ** `users.email` へ write 戻し) | △(login ID の到達検証は Keycloak 標準に委譲するのが主) |
| 氏名・住所・その他 profile・サービス属性の更新 | ✗(**read-only**。編集は本システム画面が SoT) | ✓(**主経路**。profile 更新 API) |
| user 削除 | △(admin / 障害時のみ、3.4 の論理削除に変換) | ✓(**主経路**。解約・退会 API) |

#### 属性所有ルール(2026-06-04 追加)

属性を「**ログイン ID か / 到達・所持の検証を伴う認証要素か**」で切り分ける。Yes なら Keycloak、No なら project:

- **Keycloak 所有**: 資格情報(password)、MFA factor(TOTP / WebAuthn / email-OTP。`users` に phone 列が無いため SMS-OTP は対象外)、**email**(ログイン ID、到達検証 = Keycloak Update-Email)。email は SoT として `users.email` に保管し、変更時のみ Keycloak が検証して SPI 経由で書き戻す。
- **project 所有(SPI は read-only federate)**: 氏名・住所・その他 profile・サービス属性。本システム画面が編集 SoT。Keycloak は read してトークン claim / 表示に使うのみ(read ≠ 所有・編集)。

> **「email のみ writable」の正確なスコープ**: ここでの read-only / email-only は **エンドユーザ向けの profile 属性編集**を指す(氏名・住所等は Keycloak から編集不可で project が編集 SoT、email のみ Update-Email の到達検証経由で `users.email` へ write 戻し)。これとは別に SPI は次の write 経路を保持する — (i) 突合キー `users.oidc_sub` の **初回ログイン時 correlation write**(§3.2)、(ii) **admin / 障害時リカバリの user 作成・削除**(`UserRegistrationProvider`、初期属性 set / 削除は §3.4 の論理削除・匿名化 write)。これらは日常運用ではなく correlation / 例外運用の write であり、エンドユーザの profile 編集経路ではない。
...
#### アカウント管理機能の所在(2026-06-04 追加)

資格・identity 検証に触る機能は Keycloak、ディレクトリ/profile に触る機能は本システム、で切り分ける:

- **Keycloak ホスト型(標準・無実装)**: ログイン / パスワード変更 / パスワードリセット(forgot、platform SES 経由)/ MFA(TOTP・WebAuthn・email-OTP)/ **email 変更(到達検証込み、Update-Email)**
- **本システム実装(画面要)**: 会員登録(**1 画面**で profile + サービス情報を 1 トランザクション書込、パスワードは Keycloak へ転送し本システムは保存しない)/ 氏名・住所・その他 profile・サービス属性編集 / 有効・無効・退会(ディレクトリ操作)

登録は project↔Keycloak の 2 書込にまたがる(profile = project DB、credential = Keycloak)。画面は 1 枚に保ち、順序(project 先 → Keycloak 後)+ 冪等リトライ / 補償(未完了マーク → 再設定)で吸収する。
...
- **単一 SoT**: 双方向同期(選択肢 B')の競合解決を回避し、users を **profile / directory の唯一の真実**として保つ(資格情報・MFA factor は Keycloak が SoT、§3.3)
...
- `docs/architecture/infrastructure-plan.md` v5 §5.3(Keycloak User Storage SPI)/ §3.6(Keycloak 構成)/ §3.4(Native Image との関係、Keycloak は Native 対象外)に本 ADR への参照を追加。**§6.2/§6.3 の Keycloak 行は runtime が platform へ移った前提に追従(2026-06-04 改訂)**
- **`infra/docs/adr/0004`(改訂、2026-06-04)**: Keycloak runtime(ECS Service + Custom Image + Keycloak 専用 DB + auth listener rule)を tasks → **platform 共有 IdP** へ移す(multi-realm、Custom Image は全プロジェクト SPI 同梱の共有ビルド)。`users` 表(user データ SoT)と per-realm SPI 設定は tasks 据え置き。platform→tasks 結合は「profile read + 検証済み email の write 戻し」に限定
- **Issue 波及(本 ADR 受理後)**: **#322(Keycloak ECS Service + listener rule)を tasks → platform 再配置**(2026-06-04 の Keycloak→platform 決定で、それ以前の tasks reconcile を上書き)/ #320(SPI Gradle)・#321(Custom Image)は共有 platform ビルド前提に追従 / **Keycloak 専用 DB(platform)の新規 issue 起票**
...
- Flyway migration: 既存 `V1.0.0_01__create_tables.sql` の users テーブル定義に **`version` / `deleted_at` 列を直接追記** する([設計規約 §3.1 「初期版直接追記の例外運用(dev 初回デプロイ前、Sprint 1 完了まで)」](../specs/設計規約.md#31-flyway-マイグレーション) 準拠、#248)。dev 初回デプロイ(Sprint 2 S2Infra-2)前で Flyway 履歴テーブル未生成のため新規 `V1.x.x__*.sql` を量産せず、初期版に統合する。**Sprint 0 App #90 のスコープを ADR-0006 派生作業として拡張し、docs 補正 + code (V1.0.0 への追記 / JPA エンティティ) 両方を扱う**:
  - `users` テーブル定義に `version BIGINT NOT NULL DEFAULT 0` を追加(@Version 用)
  - `users` テーブル定義に `deleted_at DATETIME NULL` を追加(論理削除日時)
...
4. **Sprint 1 Infra で Keycloak Custom Image(共有 platform ビルド)+ User Storage SPI 実装(read-only federate + email のみ writable)**(infrastructure-plan v5 §5.3 配下の Issue として起票):
...
- **SPI 単体テスト**: Testcontainers で Keycloak + MySQL を起動し、Keycloak Admin REST API 経由で SPI が users テーブルを正しく **read(profile)し、email のみ write 戻し**することを検証(資格は Keycloak ストア、name/住所等は read-only を確認)
- **E2E**:
  - Account Console から email 変更 → `users.email` に反映を確認(Update-Email 到達検証後)
  - Account Console から 氏名変更 → users テーブルに反映されないことを確認(read-only)
...
- **楽観排他**: 本システム API(profile 更新)と Keycloak Update-Email(email 変更書き戻し)から同 user を同時更新 → 一方が 409 / OptimisticLockException で失敗することを確認
