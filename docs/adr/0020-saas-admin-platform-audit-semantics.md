...
`audit_logs.tenant_id` は「**操作対象テナント**」を表すものとし、次のとおり設定する:

| 操作 | API | `tenant_id` | `entity_type` / `entity_id` |
|---|---|---|---|
| テナント停止 | A-26 `PATCH /tenants/{id}/status` | 対象テナント id | `tenant` / 対象 id |
| テナント再開 | A-26 | 対象テナント id | `tenant` / 対象 id |
| テナント名更新 | A-06 `PUT /tenants/{id}` | 対象テナント id | `tenant` / 対象 id |
| テナント詳細閲覧 | A-25 `GET /tenants/{id}` | 対象テナント id | `tenant` / 対象 id |
| テナント一覧閲覧 | A-04 `GET /tenants` | **NULL**(単一対象なし) | `tenant` / NULL |
| プラットフォームメトリクス閲覧 | A-27 `GET /platform/metrics` | **NULL**(横断集計) | NULL / NULL |
| テナント作成(セルフサインアップ) | A-05 `POST /tenants` | 作成テナント id | `tenant` / 作成 id |
