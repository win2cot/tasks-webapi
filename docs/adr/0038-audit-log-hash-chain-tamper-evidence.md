...
- **改訂**: 2026-06-25 — §3.2 を固定キー順 JSON 正準化に厳密化、§3.6 / §3.7 に B-03 アンカー境界 prefix 削除との整合を追補(瑕疵補完)
...
canonical(row) = 固定キー順 JSON{ chain_key, chain_seq, user_id, action, entity_type, entity_id, detail(ネスト), ip_address, created_at, hash_key_id }
hash_0(per chain) = GENESIS = "0" × 64
```

- `canonical(row)` は上記の全不変列を **固定キー順の正準 JSON オブジェクト**(Jackson `ORDER_MAP_ENTRIES_BY_KEYS`)に直列化し、その UTF-8 バイト列に直前行ハッシュの hex 文字列を連結して HMAC 入力とする。区切り文字連結は値内に区切りが混入すると境界が曖昧になり衝突を作れるため採らない。`NULL` 列は JSON `null` で表現する(キーは欠落させずキー集合を固定)。
- `detail` は ADR-0013 の正準化(`ORDER_MAP_ENTRIES_BY_KEYS`)に従い、`canonical(row)` ではネストした JSON オブジェクトとして埋め込む(文字列として再エスケープせず、行全体を再帰的に正準化)。
...
**B-03 はアンカー境界で prefix 削除する。** 保管期間(1 年)超の行を素朴に行単位削除すると、生存する最古行の直前行が消え、その prev ハッシュがどのアンカーにも繋がらず検証が切れる。これを避けるため B-03 は、retention cutoff より古い最新アンカー `A` を選び、`chain_key` ごとに `chain_seq ≤ A.seq_at_checkpoint` の行のみ削除する(`A` 自体と以降は残す)。生存チェーンは常に retained アンカー直後(`A.seq + 1`)から始まり、`A.head_hash` を検証起点の `hash_{n-1}` として連結する。

### 3.7 検証(B-05 バッチ)

- 毎日 02:00 にバッチ(ShedLock 排他、ADR-0037 / 基本設計書 §8.1 B-05)を実行する。
- `chain_key` ごとに、生存チェーンの起点となる retained アンカー `A`(3.6)を選び、`A.head_hash` を初期 `hash_{n-1}` として `chain_seq > A.seq_at_checkpoint` の行を末尾まで HMAC 再計算し、各行の `hash_chain` および `chain_heads.last_hash` と突合する(アンカー不在の初回は GENESIS 起点で `chain_seq = 1` から)。
