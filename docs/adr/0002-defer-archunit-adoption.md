...
- 本 ADR は **Proposed** で起票し、Sprint 0 で `ModularityTests` の運用実績を見て **Accepted へ昇格**するか、選択肢 A への切替 ADR(ADR-NNNN)を新規起票するかを判断する。
...
- 本 ADR がマージされた時点で `ModularityTests` が単一の自動検証となる。
- 再評価のトリガーが発火したら、ADR-NNNN(選択肢 A への切替)を起票する。
