...
## 11. GitHub Project + Sub-issues 運用(Milestone 廃止、2026-05-23)

### 11.1 Iteration(時間軸の単一ソース)

GitHub Project の built-in **Iteration field** で時間軸カテゴリ化を一本化。**Milestone は 2026-05-23 に全 9 個 close + 廃止**(同じ値の二重管理を解消)。
...
| `Phase 1 Sprint 2` | 2026-07-12 | 14 days | App側 + Infra 7 |
| `Phase 1 Sprint 3` | 2026-07-26 | 14 days | App側 + Infra 3 |
| `Phase 1 Sprint 4` | 2026-08-09 | 7 days | App側 + Infra 2 |
| `Phase 1 Sprint 5` | 2026-08-16 | 7 days | App側 + Infra 2 |
...
### 11.3 Tracker Issue + Sub-issues 機能(2026-05-23 採用)
...
- Tracker は **Milestone / Iteration / Stream を持たない**(cross-scope な永続 Issue)
- Phase は持つ(プロジェクト全体スコープに紐づく)
...
1. **Issue 起票時に必ず Phase + Iteration + Stream + 親 tracker への Sub-issue 関連付け を設定**(Milestone は廃止、設定不要)
2. **Iteration 進捗は週 1 回確認**(原則金曜)、ズレ 1 週間以上で計画見直し起票
3. **claude-automation 経由の Issue も Phase + Iteration + Stream を継承**(設定は gh CLI bulk script、または Web UI で個別)
4. **Project View はカスタマイズせず固定 5 view**(全 view に `phase:"Phase 1"` filter)
5. **Tracker Issue は Sub-issues 機能で hierarchy 化**、checklist 手動管理は不要(過去の checklist は段階的に削除)
6. **Cross-scope tracker(#206 等)は Milestone-less / Iteration-less / Stream-less**(Phase のみ持つ)
