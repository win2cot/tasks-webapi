...
**実装フェーズ**: Sprint 0-1 実績完了(2026-06-07)、Sprint 2-5 は **2026-06-08 〜 2026-07-19** に前倒し改訂(開発計画書 v1.4)、フェーズ 5〜7 は **2026-07-21 〜 2026-09-26** に前倒し(開発計画書 v1.5)
**担当**: win2cot(全ストリーム単独)
...
本計画書は **Phase 1 Sprint 0 着手前基盤整備フェーズ + 実装フェーズ Infra ストリーム** の現場向け実行計画であり、プロジェクト全体の公式計画書(`docs/specs/開発計画書.md` v1.5)の **補助文書** として位置付ける。

| 軸 | 開発計画書(v1.5) | 本計画書(v5) |
|---|---|---|
...
|---|---|---|
| §4.1 全体スケジュール | 実装 2026-07-14 〜 2026-09-12 / 結合 9/15-10/3 / UAT 10/6-10/31 / 本番 11/4-11/21 | 実装 Sprint 2-5: **2026-06-08 〜 2026-07-19**(v1.4)。フェーズ 5〜7: **2026-07-21 〜 2026-09-26** に前倒し(開発計画書 v1.5 §4.1) |
...
| 8 | CI/CD トリガ | **リリース = タグ駆動 / デプロイ = `workflow_dispatch`**(タグ `v*` は単一プロダクトバージョンの Release 生成のみ起動。デプロイは version + environment を選ぶ dispatch、[ADR-0031](../adr/0031-single-product-version-dispatch-deploy.md)) |
| 9 | シークレット管理 | **Parameter Store SecureString**(Secrets Manager は不使用、コスト面) |
| 10 | tag 命名 | **単一プロダクトバージョン `vX.Y.Z`**(component prefix・環境 suffix なし。リポジトリ全体で 1 本の線形 Release 列、SemVer 2.0。変更コンポーネントのみビルド/デプロイは変更検知で判定、[ADR-0031](../adr/0031-single-product-version-dispatch-deploy.md)) |
...
| 18 | スケジュール | 基盤整備 3 週間 + 実装フェーズ(Sprint 0-1 実績完了 2026-06-07、Sprint 2-5: 2026-06-08 〜 2026-07-19 前倒し)。**Phase 1 Sprint 0 開始 2026-06-14**(v4 の 7/14 から 1 ヶ月前倒し)、v1.5 でさらに前倒し(フェーズ 5〜7 含む) |
...
flowchart TB
  ...
  subgraph gha["gha: GitHub Actions CI（OIDC）"]
...
ECR には `keycloak-custom` リポジトリを別途用意し、環境非依存の `vX.Y.Z` tag で push → ECS Service が pull(build once / promote digest、[ADR-0028](../adr/0028-build-once-promote-digest.md))。
...
W2-W3   Phase 1 Sprint 2 (2w、6/08〜6/21)       ← v1.4 前倒し
        ...
        └─ Phase 1 Sprint 2 Infra:  リリース生成 wf + dispatch デプロイ wf(ADR-0031)
...
| # | サブタスク | claude-automation 適用 | Iteration |
|---|---|---|---|
| S2Infra-1 | infra(app): tasks-webapi コンテナイメージビルド(bootBuildImage / Cloud Native Buildpacks、Native Image、IAM 認証クライアント設定込み) | 適用 | Phase 1 Sprint 2 |
...
> **[ADR-0031 注記]** 本節 S2Infra-4〜7 は旧 tag 駆動設計(#481-484、close 済)。デプロイ/リリースは [ADR-0031](../adr/0031-single-product-version-dispatch-deploy.md) で単一プロダクトバージョン + `workflow_dispatch` に再設計し、実装は #634(リリース生成)/ #635(デプロイ)に集約。`infra-v*` タグトリガは #637 で撤去済。

> **[S2Infra-1 実装時の注意]** イメージ作成手段は [ADR-0018](../adr/0018-container-image-build-with-boot-build-image.md) で `bootBuildImage`(Cloud Native Buildpacks)に確定し、Dockerfile は作成しない。`./gradlew :webapi:bootBuildImage` で `tasks-webapi:0.0.1-SNAPSHOT` イメージを生成する。ECR push フローは `docker tag` + `docker push` で行う。
>
> **[S2Infra-4〜7 実装時の注意]** デプロイ tag は [ADR-0026](../adr/0026-component-scoped-deploy-tags.md) のコンポーネント別 prefix 形式(`webapi-v*` / `keycloak-v*` / `web-v*` / `infra-v*`)で各 workflow を振り分ける。S2Infra-7(infra)は plan job(summary 出力 + tfplan artifact 保存)→ GitHub Environments required reviewers 承認 → apply job(保存済み tfplan を適用)の 2 段構成とする。ECR のイメージ tag は環境 suffix を含まない `vX.Y.Z`、イメージ build は version につき 1 回(`-dev` tag push 時)のみ、stg/prd は digest promote([ADR-0028](../adr/0028-build-once-promote-digest.md))。

#### Phase 1 Sprint 2 受入条件(統合)

- [ ] tasks-webapi が dev で動作、API 28 endpoints 全て応答
- [ ] `v0.1.0` タグ push で変更分のみビルド + Release 生成、`workflow_dispatch`(version+environment)で dev へデプロイ(ADR-0031)
...
| S3Infra-4 | infra(scheduling): EventBridge Scheduler で dev ECS + RDS の稼働時間切替(平日 19:00-02:00 / 土日 10:00-02:00、Lambda + IAM 経由 `ecs:UpdateService` / `rds:StartDBInstance`・`rds:StopDBInstance`)| 適用 | Phase 1 Sprint 3 (先行実装: #620) |
...
| ログ基盤選定 ADR | **確定済**(infra ADR-0005, 2026-06-06: Amazon CloudWatch Logs を正式採用)。Phase 1 Sprint 3 Infra S3Infra-3 で実装(#671) |
| NIST SP 800-53 MVP scope 確定 | Phase 1 Setup 1 中に初期下書き、Phase 1 Sprint 4 Infra S4Infra-2 で本格整備 |

純粋な「並行ストリーム」は **ログ基盤選定 ADR の方針確定タイミング** のみ残っていたが、これも下記のとおり確定済:

| # | サブタスク | claude-automation 適用 | Iteration | 状態 |
|---|---|---|---|---|
| P-LOG-A | design(logging): ログ基盤選定 ADR(`infra/docs/adr/` 配下) | 半適用 | Phase 1 Sprint 1 (期限) | **完了**(infra ADR-0005, CloudWatch Logs 正式採用)。実装は S3Infra-3 / #671 |
...
| # | 起票順 | タイトル(案) | 優先度 | Iteration |
|---|---|---|---|---|
| S2Infra-1 | 31 | infra(app): tasks-webapi コンテナイメージビルド(bootBuildImage / Cloud Native Buildpacks、Native Image) | p0 | Phase 1 Sprint 2 |
...
| S3Infra-4 | 41 (#620) | infra(scheduling): EventBridge Scheduler で dev ECS + RDS 稼働時間切替(平日 19:00-02:00 + 土日 10:00-02:00) | p1 | Phase 1 Sprint 3 (先行実装) |
...
- [ ] `v0.1.0` タグ push で変更コンポーネントのみビルド + Release 生成(ADR-0031)
- [ ] `workflow_dispatch`(version=v0.1.0, env=dev)で差分コンポーネントを dev へデプロイ + Deployments 記録
- [ ] infra は dispatch で plan → Environments 承認 → `terraform apply`(ADR-0031)
...
| AWS リソースコスト想定超 | **Phase 1 Sprint 3 Infra S3Infra-4(#620)** で EventBridge Scheduler を構築。dev ECS Service の desiredCount と RDS の start/stop を **平日 19:00-02:00 + 土日 10:00-02:00**(副業稼働時間に合わせ)に絞る。3 cron(平日 19:00 起動 / 週末 10:00 起動 / 毎日 02:00 停止)で Lambda 経由切替。起動時は RDS start → available 待機 → ECS 起動、停止時は ECS 停止 → RDS stop の順序制御。ECS 部分コスト削減: 約 $18/月($30→$12)、RDS コンピュート削減: 約 $11/月(稼働率約40%相当、ストレージは停止中も課金)。NAT Gateway は常時稼働(固定コスト)。なお「RDS は最大7日停止制約のため停止不可」は誤り: 7日制約は連続停止が7日を超える場合のみ適用され、日次起動スケジュールでは無関係。Phase 1 Sprint 0-2 までは構築中のため 24h 稼働(短期 $90/月 程度) |
| Keycloak Custom SPI 実装が難航 | Phase 1 Sprint 1 Infra で公式 Keycloak Image 採用、Custom Image は Phase 1 Sprint 3-4 へ延期 |
| ログ基盤の選定が長引く | **解消済**: infra ADR-0005(2026-06-06)で 4 候補を比較のうえ CloudWatch Logs を**正式採用**(暫定回避ではない)。実装は S3Infra-3 / #671 |
...
| v5.4 | 2026-06-12 | [ADR-0026](../adr/0026-component-scoped-deploy-tags.md) 反映: tag 命名をコンポーネント別 prefix 形式(`<component>-vX.Y.Z` + 環境 suffix、§1 項目 8 / 10)に確定し、git tag 名前空間の衝突(旧仕様はコンポーネント識別なし)を解消。§6.3 に S2Infra-4〜7 実装時の注意(tag pattern 振り分け + infra は plan → Environments 承認 → apply の 2 段構成)を追加、§6.3 / §9 受入条件の tag 例を `webapi-v0.1.0-dev` 等に整合 |
| v5.5 | 2026-06-12 | [ADR-0028](../adr/0028-build-once-promote-digest.md) 反映: イメージは build once / promote digest(ECR イメージ tag は環境非依存の `vX.Y.Z`、昇格 tag は再 build しない)。§3.7 keycloak ECR push と §6.3 S2Infra-4〜7 注意書きの ECR tag 文言を修正 |
| v5.6 | 2026-06-14 | [ADR-0031](../adr/0031-single-product-version-dispatch-deploy.md) 反映(ADR-0026/0030 supersede・0028 amend): デプロイ/リリースを単一プロダクトバージョン `vX.Y.Z` + `workflow_dispatch`(version+environment)+ GitHub Deployments に再設計。§1 項目 8/10 改訂、§6 S2Infra-4〜7 に再設計注記(実装 #634/#635)、受入条件の tag push 表現を更新。component 別 prefix tag・環境 suffix tag・tag 駆動デプロイは廃止 |
