...
**実装フェーズ**: 10 週間(2026-06-14 〜 2026-08-22、Phase 1 Sprint 0-5、App + Infra 並行 2 ストリーム)
**担当**: win2cot(全ストリーム単独)
...
本計画書は **Phase 1 Sprint 0 着手前基盤整備フェーズ + 実装フェーズ Infra ストリーム** の現場向け実行計画であり、プロジェクト全体の公式計画書(`docs/specs/開発計画書.md` v1.3)の **補助文書** として位置付ける。

| 軸 | 開発計画書(v1.3) | 本計画書(v5) |
|---|---|---|
...
| 粒度 | フェーズ単位 / 役割単位 / コスト単位 | Setup / Sprint 単位 / Issue 単位 / Milestone 単位 |
...
|---|---|---|
| §4.1 全体スケジュール | 実装 2026-07-14 〜 2026-09-12 / 結合 9/15-10/3 / UAT 10/6-10/31 / 本番 11/4-11/21 | 実装 **2026-06-14 〜 2026-08-22**(基盤整備 5/23-6/13 を前置)/ 結合 8/24-9/11 / UAT 9/14-10/9 / 本番 10/13-10/30 |
...
| 18 | スケジュール | 基盤整備 3 週間 + 実装 10 週間 = 13 週間(2026-05-23 〜 2026-08-22)。**Phase 1 Sprint 0 開始 2026-06-14**(v4 の 7/14 から 1 ヶ月前倒し) |
...
```
┌──────────────────────────────────────────────────────────────┐
│ local: Developer Machine(WSL Ubuntu 24)                    │
│   docker-compose.local.yml                                    │
│   ├─ MySQL 8.4 (port 3306)                                   │
│   └─ Keycloak 24+(local は公式 image、dev は Custom Image) │
│   tasks-webapi(host 起動)+ web/(静的配信)                 │
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ gha: GitHub Actions CI 環境                                  │
│   tasks-webapi / keycloak / web / infra: build / test /      │
│   deploy(tag 駆動、OIDC AWS 認証)                           │
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ dev: AWS(ap-northeast-1、tasks.dgz48.xyz 系ドメイン)       │
│   稼働時間: 平日 19:00-02:00 + 土日 10:00-02:00(EB 切替)    │
│                                                                │
│   Route53(dgz48.xyz、Public)─┬─ tasks-dev.dgz48.xyz → CFront│
│                                 ├─ api-dev.tasks.dgz48.xyz → ALB│
│                                 └─ auth-dev.tasks.dgz48.xyz→ ALB│
│   Route53 PHZ "tasks.internal"(Private、dev VPC association) │
│                              └─ db.tasks.internal → RDS endpoint│
│   VPC(2 AZ)                                                 │
│      ├─ Public  : ALB(single、Host-based Rule)             │
│      └─ Private :                                            │
│          ├─ ECS Fargate(tasks-webapi service)              │
│          │   └─ DATASOURCE_URL=jdbc:mysql://db.tasks.internal│
│          │   └─ RDS 接続は IAM 認証                         │
│          ├─ ECS Fargate(Keycloak Custom service)           │
│          │   └─ RDS 接続はパスワード認証(K-A)             │
│          └─ RDS MySQL 8.4(db.t4g.micro、Single AZ)        │
│              ├─ master user (Parameter Store、DBA 業務用)   │
│              ├─ tasks_webapi user (IAM 認証)                │
│              └─ keycloak user (Parameter Store)             │
│   ECR(tasks-webapi + keycloak-custom)                       │
│   Parameter Store SecureString                                │
│   ログ基盤(別議論)/ S3 + DynamoDB(Terraform state)       │
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ stg / prd: MVP リリース時は構築しない                        │
│   Terraform 設計も Post-Sprint-0 に延期                      │
└──────────────────────────────────────────────────────────────┘
```
...
`webapi/src/main/resources/application.yml` は単一。環境差は ECS Task Definition の `environment` / `secrets` で注入。詳細は v4 §3.4 を保持。
...
```
[dev VPC]
tasks-webapi container ─ env DATASOURCE_URL=jdbc:mysql://db.tasks.internal:3306/tasks
                          │
                          ▼ (VPC DNS resolver)
...
[stg / prd VPC(将来)]
同様に PHZ "tasks.internal" が独立して作成、db.tasks.internal の参照先は当該 env の RDS
```
...
独立 Gradle project として `src/main/java/` に User Storage SPI 等を実装、`META-INF/services/` で SPI 登録、`build.gradle` で JAR 生成。`keycloak/Dockerfile` は公式 Keycloak image + 自作 SPI JAR の二段ビルド。詳細は v4 §3.6 を保持。

「DBA: 既存 users 参照」は **この SPI から既存 users テーブルへ read 接続する** 形になる(**Phase 1 Setup 2** で ADR 化)。
...
[実装フェーズ(10 週間、2026-06-14 〜 2026-08-22) — App + Infra 並行]

W3-W4   Phase 1 Sprint 0 (2w、6/14〜6/27)
...
W5-W6   Phase 1 Sprint 1 (2w、6/28〜7/11)
...
|---|---|---|---|
| Setup0-1 | infra(repo): monorepo 構造整理 PR(`webapi/` 等 subdir + 資材移動 + CI workflow パス更新) | 半適用 | M-Setup-0 |
| Setup0-2 | infra(ci): Renovate `packageRules` 多 path 対応(`webapi/**` `keycloak/**` `infra/**` `web/**`) | 適用 | M-Setup-0 |
| Setup0-3 | docs(architecture): 本計画書(v5)+ tracker Issue を commit | 適用 | M-Setup-0 |
...
| # | サブタスク | claude-automation 適用 | Milestone |
|---|---|---|---|
| Setup1-1 | infra(compose): `docker-compose.local.yml` 雛形(MySQL 8.4 + Keycloak 公式 image) | 適用 | M-Setup-1 |
| Setup1-2 | infra(compose): Keycloak Realm 設計(`tasks` realm、Roles、Test Users、Client)+ JSON export 配置(`keycloak/realm-export/`) | 半適用 | M-Setup-1 |
| Setup1-3 | infra(compose): MySQL 初期化 + Flyway 起動確認 | 適用 | M-Setup-1 |
| Setup1-4 | infra(app): `application.yml` 単一化 + env vars 注入の枠組み | 適用 | M-Setup-1 |
| Setup1-5 | infra(web): `web/` ディレクトリ雛形(`index.html` + Bootstrap 5 + 認証 JS) | 適用 | M-Setup-1 |
| Setup1-6 | docs(dev): ローカル開発セットアップガイド(`docs/dev/local-setup.md`) | 適用 | M-Setup-1 |
| Setup1-7 | infra(compose): E2E 動作確認手順 + ログ証跡 | 半適用 | M-Setup-1 |
...
| ADR | 配置 | 議論ポイント |
|---|---|---|
| Keycloak User Storage SPI 設計 | `docs/adr/0005-keycloak-user-storage-spi.md`(横断) | 対象 DB / 接続経路(直接 JDBC or 既存 API)/ 更新方向(初期 read のみ)/ 同期方針 / API 互換性 |
| RDS MySQL vs Aurora | `docs/adr/0006-rds-mysql-vs-aurora.md`(横断) | dev は RDS MySQL 8.4 で確定(コスト ~$15/月 vs Aurora ~$43/月、testcontainers 完全互換、ベンダーロックイン回避)。prd 構築時(Post-Sprint-0)に再評価 |
| GraalVM Native Image 採用 | `docs/adr/0007-graalvm-native-image.md`(横断) | Fat JAR とのトレードオフ(起動時間 / メモリ / ビルド複雑化 / Reflection 制約 / `@RegisterReflectionForBinding` 等)。Spring プロファイル不使用の理由を本 ADR で集約 |
| Private DNS for RDS | `infra/docs/adr/0001-private-dns-for-rds.md`(IaC 固有) | Route53 Private Hosted Zone で `db.tasks-dev.internal` 等の CNAME を被せ、Aurora 移行 / エンジン切替時のアプリ ENV 不変を実現 |

(ADR 番号は既存 0001-0004 + ADR-0005 §6 のあとに続く想定。実起票時に競合確認)

#### サブタスク

| # | サブタスク | claude-automation 適用 | Milestone |
|---|---|---|---|
| Setup2-1 | design(dba/keycloak): Keycloak User Storage SPI 設計 ADR | 半適用 | M-Setup-2 |
| Setup2-2 | design(db): RDS MySQL vs Aurora 選定 ADR | 半適用 | M-Setup-2 |
| Setup2-3 | design(network): Private DNS for RDS 採否 ADR(IaC 固有、`infra/docs/adr/`) | 半適用 | M-Setup-2 |
| Setup2-4 | design(runtime): GraalVM Native Image 採用 ADR + `docs/specs/コーディング規約.md` に Native Image 実装制約セクション追記(Reflection ヒント / `@RegisterReflectionForBinding` 等) | 半適用 | M-Setup-2 |
...
| # | サブタスク | claude-automation 適用 | Milestone |
|---|---|---|---|
| S0Infra-1 | infra(terraform): Terraform プロジェクト構造 ADR(`infra/modules/` `infra/environments/` `infra/docs/adr/`) | 半適用 | Phase 1 Sprint 0 |
| S0Infra-2 | infra(terraform): state backend(S3 + DynamoDB)構築 + bootstrap 手順 | 半適用(初回手動) | Phase 1 Sprint 0 |
| S0Infra-3 | infra(ci-cd): GitHub Actions OIDC + IAM Role(`terraform apply` 用) | 半適用 | Phase 1 Sprint 0 |
| S0Infra-4 | infra(terraform): VPC + Subnets(public/private)+ IGW + NAT + Route Tables | 適用 | Phase 1 Sprint 0 |
...
| S0Infra-7 | infra(terraform): Parameter Store SecureString パラメータ整備 + `docs/architecture/parameter-store.md` 横断 doc 同時 commit(全 PS パラメータの階層 / 型 / アクセス許可者 / 回転ポリシー 一覧) | 適用 | Phase 1 Sprint 0 |
| S0Infra-8 | infra(terraform): Route53 Private Hosted Zone(`tasks-dev.internal`)+ VPC association(Setup2-3 ADR 反映、RDS への CNAME placeholder) | 適用 | Phase 1 Sprint 0 |
...
| # | サブタスク | claude-automation 適用 | Milestone |
|---|---|---|---|
| S1Infra-1 | infra(terraform): RDS MySQL 8.4(dev、db.t4g.micro、**IAM 認証有効化**)+ Private Hosted Zone へ CNAME(`db.tasks-dev.internal` → RDS endpoint)登録 | 適用 | Phase 1 Sprint 1 |
...
### 6.3 Phase 1 Sprint 2(W7-W8、2026-07-12 〜 2026-07-25、2 週間)
...
| S2Infra-5 | infra(ci-cd): keycloak custom tag → ECR push + ECS update | 適用 | Phase 1 Sprint 2 |
...
### 6.4 Phase 1 Sprint 3(W9-W10、2026-07-26 〜 2026-08-08、2 週間)
...
### 6.5 Phase 1 Sprint 4(W11、2026-08-09 〜 2026-08-15、1 週間)
...
### 6.6 Phase 1 Sprint 5(W12、2026-08-16 〜 2026-08-22、1 週間)
...
すべて tasks-webapi リポへ起票。Milestone は **M-Setup-0/1/2** または **Phase 1 Sprint 0..5**。

### 8.1 基盤整備フェーズ(Phase 1 Setup 0/1/2、14 件)

| # | 起票順 | タイトル(案) | 優先度 | Milestone |
|---|---|---|---|---|
| Setup0-1 | 1 | infra(repo): monorepo 構造整理 PR | p0 | M-Setup-0 |
| Setup0-2 | 2 | infra(ci): Renovate packageRules 多 path 対応 | p1 | M-Setup-0 |
| Setup0-3 | 3 | docs(architecture): infrastructure-plan.md v5 commit + tracker Issue 起票 | p1 | M-Setup-0 |
| Setup1-1 | 4 | infra(compose): MySQL 8.4 + Keycloak の Docker Compose スタック雛形 | p0 | M-Setup-1 |
| Setup1-2 | 5 | infra(compose): Keycloak Realm 設計 + JSON export 配置 | p0 | M-Setup-1 |
| Setup1-3 | 6 | infra(compose): MySQL 初期化 + Flyway 起動確認 | p1 | M-Setup-1 |
| Setup1-4 | 7 | infra(app): `application.yml` 単一化 + env vars 注入の枠組み | p0 | M-Setup-1 |
| Setup1-5 | 8 | infra(web): `web/` ディレクトリ雛形 + Bootstrap 5 + 認証 JS | p1 | M-Setup-1 |
| Setup1-6 | 9 | docs(dev): ローカル開発セットアップガイド | p2 | M-Setup-1 |
| Setup1-7 | 10 | infra(compose): E2E 動作確認手順 + ログ証跡 | p2 | M-Setup-1 |
| Setup2-1 | 11 | design(dba/keycloak): Keycloak User Storage SPI 設計 ADR(`docs/adr/0005-...`) | p0 | M-Setup-2 |
| Setup2-2 | 12 | design(db): RDS MySQL vs Aurora 選定 ADR(`docs/adr/0006-...`) | p0 | M-Setup-2 |
| Setup2-3 | 13 | design(network): Private DNS for RDS 採否 ADR(`infra/docs/adr/0001-...`) | p0 | M-Setup-2 |
| Setup2-4 | 14 | design(runtime): GraalVM Native Image 採用 ADR(`docs/adr/0007-...`)+ コーディング規約に Native Image 実装制約追記 | p0 | M-Setup-2 |

### 8.2 Phase 1 Sprint 0 Infra(8 件、W3-W4)

| # | 起票順 | タイトル(案) | 優先度 | Milestone |
|---|---|---|---|---|
| S0Infra-1 | 15 | infra(terraform): Terraform プロジェクト構造 ADR | p0 | Phase 1 Sprint 0 |
| S0Infra-2 | 16 | infra(terraform): state backend(S3 + DynamoDB)構築 | p0 | Phase 1 Sprint 0 |
| S0Infra-3 | 17 | infra(ci-cd): GitHub Actions OIDC + IAM Role | p0 | Phase 1 Sprint 0 |
| S0Infra-4 | 18 | infra(terraform): VPC + Subnets + Routing | p0 | Phase 1 Sprint 0 |
...
| S0Infra-8 | 22 | infra(terraform): Route53 Private Hosted Zone(`tasks-dev.internal`)+ VPC association | p0 | Phase 1 Sprint 0 |

### 8.3 Phase 1 Sprint 1 Infra(8 件、W5-W6)

| # | 起票順 | タイトル(案) | 優先度 | Milestone |
|---|---|---|---|---|
| S1Infra-1 | 23 | infra(terraform): RDS MySQL 8.4(IAM 認証有効化)+ PHZ CNAME(`db.tasks-dev.internal`) | p0 | Phase 1 Sprint 1 |
...
| S2Infra-5 | 35 | infra(ci-cd): keycloak custom tag → ECR push + ECS update | p0 | Phase 1 Sprint 2 |
...
- [ ] Phase 1 Setup 0-2 サブ Issue がすべてクローズ(M-Setup-0/1/2 100%)
...
## 11. Milestone / GitHub Project 運用

### 11.1 Milestone

Setup 単位 + Sprint 単位で Milestone を切る。各 Milestone に **due_date** と **受入条件(本文に要点)** を設定。

| Milestone 名 | due_date | 紐づく Issue 数(目安) |
|---|---|---|
| `Phase 1 Setup 0`: monorepo 構造整理 | 2026-05-30 | 3 |
| `Phase 1 Setup 1`: ローカル開発環境 | 2026-06-13 | 7 |
| `Phase 1 Setup 2`: Keycloak SPI 設計 ADR | 2026-06-13 | 1 |
| `Phase 1 Sprint 0`: scaffold 整合 + Terraform 基盤 | 2026-06-27 | App側(#121 配下) + Infra 7 |
| `Phase 1 Sprint 1`: テナント切替 + AWS dev リソース | 2026-07-11 | App側 + Infra 8 |
| `Phase 1 Sprint 2`: タスク CRUD 仕上げ + deploy CI/CD | 2026-07-25 | App側 + Infra 7 |
| `Phase 1 Sprint 3`: 機能拡張 + dev E2E + ログ基盤 | 2026-08-08 | App側 + Infra 3 |
| `Phase 1 Sprint 4`: ダッシュボード + runbook + NIST | 2026-08-15 | App側 + Infra 2 |
| `Phase 1 Sprint 5`: 漏洩テスト + 最終仕上げ | 2026-08-22 | App側 + Infra 2 |

(統括 Milestone は GitHub Milestone の親子関係未サポートなので Tracker Issue + 統括ラベルで代替)

### 11.2 GitHub Project(GH Projects v2)

**既存の Project を使用**(リポジトリ または ユーザレベル、既設定なし)。本計画書で以下の構成を提案:
...
- Issue が `priority/p0` ラベル付きで起票 → `Priority = p0` 自動セット
- Issue クローズ → `Status = Done`
- PR が Draft マージ(claude-automation の done パス)→ `Status = Done`(該当 Issue 側)
...
5. **Sprint X Infra Issue は本計画書 Tracker + Sprint X トラッカの両方から参照**(同じ Issue を 2 つのトラッカでチェックリスト管理)
...
| **v5** | **2026-05-23** | **構造再編: Stage → Setup(基盤整備 3 週間)+ Sprint X App/Infra(実装 10 週間)に統一。Phase 1 Sprint 0 開始を 2026-07-14 → 2026-06-14 に 1 ヶ月前倒し。DBA SPI ADR を Phase 1 Setup 2 に格上げ。Milestone 9 個構成(M-Setup-0/1/2 + Phase 1 Sprint 0..5)。MVP リリース見込み 2026-10 月下旬** |
