...
| 8 | CI/CD トリガ | **tag 駆動**(コンポーネント別 prefix tag。`webapi-v*` / `keycloak-v*` / `web-v*` / `infra-v*` で workflow を振り分け、[ADR-0026](../adr/0026-component-scoped-deploy-tags.md)) |
| 9 | シークレット管理 | **Parameter Store SecureString**(Secrets Manager は不使用、コスト面) |
| 10 | tag 命名 | **`<component>-vX.Y.Z` + 環境 suffix**(component = `webapi` / `keycloak` / `web` / `infra`、suffix = `-dev` / `-stg` / なし(= prd)。例: `webapi-v1.0.0-dev`。SemVer 2.0 整合、各コンポーネント独立採番、[ADR-0026](../adr/0026-component-scoped-deploy-tags.md)) |
...
- [ ] コンポーネント別 tag push(`webapi-v*` / `keycloak-v*` / `web-v*` / `infra-v*`)で対応ストリームのみ自動デプロイ(ADR-0026)
...
- [ ] `webapi-v0.1.0-dev` tag push で ECR push + ECS update 自動実行
- [ ] `keycloak-v0.1.0-dev` tag push で ECR push + ECS update 自動実行
- [ ] `infra-v0.1.0-dev` tag push で plan → Environments 承認 → `terraform apply` 自動実行(ADR-0026)
