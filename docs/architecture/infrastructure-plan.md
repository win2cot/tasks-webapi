...
| # | サブタスク | claude-automation 適用 | Milestone |
|---|---|---|---|
| Setup1-1 | infra(compose): `docker-compose.local.yml` 雛形(MySQL 8.4 + Keycloak 公式 image) | 適用 | Phase 1 Setup 1 |
| Setup1-2 | infra(compose): Keycloak Realm 設計(`tasks` realm、Roles、Test Users、Client)+ JSON export 配置(`keycloak/realm-export/`) | 半適用 | Phase 1 Setup 1 |
| Setup1-3 | infra(compose): MySQL 初期化 + Flyway 起動確認 | 適用 | Phase 1 Setup 1 |
| Setup1-4 | infra(app): `application.yml` 単一化 + env vars 注入の枠組み | 適用 | Phase 1 Setup 1 |
| Setup1-5 | infra(web): `web/` ディレクトリ雛形(`index.html` + Bootstrap 5 + 認証 JS) | 適用 | Phase 1 Setup 1 |
| Setup1-6 | docs(dev): ローカル開発セットアップガイド(`docs/dev/local-setup.md`) | 適用 | Phase 1 Setup 1 |
| Setup1-7 | infra(compose): E2E 動作確認手順 + ログ証跡 | 半適用 | Phase 1 Setup 1 |
...
| # | サブタスク | claude-automation 適用 | Milestone |
|---|---|---|---|
| Setup2-1 | design(dba/keycloak): Keycloak User Storage SPI 設計 ADR | 半適用 | Phase 1 Setup 2 |
| Setup2-2 | design(db): RDS MySQL vs Aurora 選定 ADR | 半適用 | Phase 1 Setup 2 |
| Setup2-3 | design(network): Private DNS for RDS 採否 ADR(IaC 固有、`infra/docs/adr/`) | 半適用 | Phase 1 Setup 2 |
| Setup2-4 | design(runtime): GraalVM Native Image 採用 ADR + `docs/specs/コーディング規約.md` に Native Image 実装制約セクション追記(Reflection ヒント / `@RegisterReflectionForBinding` 等) | 半適用 | Phase 1 Setup 2 |
...
すべて tasks-webapi リポへ起票。Milestone は **Phase 1 Setup 0/1/2** または **Phase 1 Sprint 0..5**。
...
| Setup1-1 | 4 | infra(compose): MySQL 8.4 + Keycloak の Docker Compose スタック雛形 | p0 | Phase 1 Setup 1 |
| Setup1-2 | 5 | infra(compose): Keycloak Realm 設計 + JSON export 配置 | p0 | Phase 1 Setup 1 |
| Setup1-3 | 6 | infra(compose): MySQL 初期化 + Flyway 起動確認 | p1 | Phase 1 Setup 1 |
| Setup1-4 | 7 | infra(app): `application.yml` 単一化 + env vars 注入の枠組み | p0 | Phase 1 Setup 1 |
| Setup1-5 | 8 | infra(web): `web/` ディレクトリ雛形 + Bootstrap 5 + 認証 JS | p1 | Phase 1 Setup 1 |
| Setup1-6 | 9 | docs(dev): ローカル開発セットアップガイド | p2 | Phase 1 Setup 1 |
| Setup1-7 | 10 | infra(compose): E2E 動作確認手順 + ログ証跡 | p2 | Phase 1 Setup 1 |
| Setup2-1 | 11 | design(dba/keycloak): Keycloak User Storage SPI 設計 ADR(`docs/adr/0006-...`) | p0 | Phase 1 Setup 2 |
| Setup2-2 | 12 | design(db): RDS MySQL vs Aurora 選定 ADR(`docs/adr/0007-...`) | p0 | Phase 1 Setup 2 |
| Setup2-3 | 13 | design(network): Private DNS for RDS 採否 ADR(`infra/docs/adr/0001-...`) | p0 | Phase 1 Setup 2 |
| Setup2-4 | 14 | design(runtime): GraalVM Native Image 採用 ADR(`docs/adr/0008-...`)+ コーディング規約に Native Image 実装制約追記 | p0 | Phase 1 Setup 2 |
