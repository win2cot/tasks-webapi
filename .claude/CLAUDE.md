...
- `docs/specs/Terraform規約.md` — IaC rules for `infra/**/*.tf`: IAM least privilege (no wildcard actions except read-only `Get*`/`List*`/`Describe*` prefixes, ARN-scoped resources, IAM permissions added in the same PR as the resource they serve). Naming/tagging SSOT is infra ADR-0004 §D (supersedes ADR-0002 §3.F).
