...
class TaskControllerWebMvcTest {
  ...
  void getTask_returns200WithTask_whenAuthenticated() throws Exception {
    ...
    Task task =
        new Task(
            ...
            now,
            now);
    when(getTaskUseCase.getTask(eq(1L), eq(1L))).thenReturn(task);

    mockMvc
        .perform(get("/api/tasks/1").header("X-Tenant-Id", "1").with(jwt()))
  ...
  void getTask_returns404_whenNotFound() throws Exception {
    when(getTaskUseCase.getTask(eq(1L), eq(99L))).thenThrow(new TaskNotFoundException(99L));

    mockMvc
        .perform(get("/api/tasks/99").header("X-Tenant-Id", "1").with(jwt()))
